package cn.conststar.wall.service.verifyCode;
...
import cn.conststar.wall.utils.VerifyCode;
...
public class VerifyCodeService {
    ...
    private static Map<String, VerifyCode> verifyCodeMap = new HashMap<>();
    ...
    public static int getRotateImage(ByteArrayOutputStream out) throws IOException {
        File imageDirectory = Tools.getConfCodeImageDirectory();
        ...
        assert list != null;
        if (list.length == 0) {
            throw new BusinessException("图片库为空", ResponseEnumConstant.CODE_50001);
        }
        ...
        BufferedImage image = ImageUtils.rotate(imageFile, angel);
    ...
    //获取验证码对象
    public VerifyCode getVerifyCode(@Email(message = "邮箱格式错误") String email) {
    ...
    public String getRotationVerifyCode(String email) throws Exception {
        VerifyCode verifyCode = getVerifyCode(email);

        ByteArrayOutputStream out = new ByteArrayOutputStream();
        int angle = VerifyCodeService.getRotateImage(out);
        verifyCode.setRotateCodeArgs(angle);

        return ImageUtils.base64(out, "png");
    }
    ...
    public void getEmailVerifyCode(String email, int angle) throws Exception {
        ...
        //验证旋转验证码
        verifyCode.verifyRotateCode(angle);

        //发送邮箱验证码
        verifyCode.sendEmailCode(email);
    }

    //验证 邮箱验证码
    public void verifyEmailCode(String email, String code) {
        VerifyCode verifyCode = getVerifyCode(email);
        verifyCode.verifyEmailCode(code);
    }
